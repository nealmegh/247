...
{{--        <h3 class="in-heading align-self-center">{{$siteSettings[9]->value}}</h3>--}}
    ...
    <div class="w-50 float-right mt-10">
        <p class="m-0 pt-5 text-bold w-100">Bill: <span class="" style="color: dodgerblue">{{$bill->id}}</span></p>
        <p class="m-0 pt-5 text-bold w-100">Invoice Date: <span class="gray-color">{{date('d M Y', strtotime($bill->created_at))}}</span></p>
...
<div class="float-left w-100" style="padding-top: 20px !important;">
    ...
    <p class="m-0 pt-5 w-100">Name: {{$bill->invoices[0]->trip->driver->name}}</p>
    <p class="m-0 pt-5 w-100">Registration: {{$bill->invoices[0]->trip->driver->vehicle_reg}}</p>
    <p class="m-0 pt-5 w-100">Phone: {{$bill->invoices[0]->trip->driver->phone_number}}</p>
</div>
